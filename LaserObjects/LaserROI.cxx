...
lasercal::LaserROI::LaserROI(const float &BoxSize, const lasercal::LaserBeam &LaserBeamInfo) : fBoxSize(BoxSize),
                                                                                               fLaserBeam(
                                                                                                       LaserBeamInfo) {
    fGeometry = &*(art::ServiceHandle<geo::Geometry>());
...
//     ExitPoint.Print();
    ...
    std::cout << "entry: ";
    fLaserBeam.GetEntryPoint().Print();
    std::cout << "exit:  ";
    fLaserBeam.GetExitPoint().Print();
    ...
    for (unsigned int plane_no = 0; plane_no < fGeometry->Nplanes(); plane_no++) {
        ...
        for (unsigned int wire_no = BoxRanges.at(plane_no).first.Wire;
             wire_no <= BoxRanges.at(plane_no).second.Wire; wire_no++) {
            ...
            std::pair<float, float> TickLimitsOfWire = std::make_pair(CentralX - fXScaleFactor,
                                                                      CentralX + fXScaleFactor);
        ...
        auto entry = fRanges.at(plane_no).begin();
        auto exit = fRanges.at(plane_no).rbegin();

        std::cout << "ROI at plane " << plane_no << std::endl;
        std::cout << "ROI Entry: wire / start_tick / end_tick " << entry->first << "/" << entry->second.first << "/" << entry->second.second << std::endl;
        std::cout << "ROI Exit:  wire / start_tick / end_tick " << exit->first << "/" << exit->second.first << "/" << exit->second.second << std::endl;
...
void lasercal::LaserROI::setRanges(int BoxTickCenter, int BoxTickWidth, unsigned int Plane,
                                   std::pair<unsigned int, unsigned int> Wires) {

    std::pair<float, float> TickLimitsOfWire = std::make_pair(float(BoxTickCenter - BoxTickWidth / 2),
                                                              float(BoxTickCenter + BoxTickWidth / 2));

    if (Wires.second < Wires.first) Wires.first, Wires.second = Wires.second, Wires.first;
...
float lasercal::LaserROI::GetEntryTimeTick(const unsigned int &PlaneNo) const {
    return 0.5 * (fRanges.at(PlaneNo).find(fEntryWire.at(PlaneNo))->second.first +
                  fRanges.at(PlaneNo).find(fExitWire.at(PlaneNo))->second.second);
}

//----------------------------------------------------------------------------------------------------------------

float lasercal::LaserROI::GetExitTimeTick(const unsigned int &PlaneNo) const {
    return 0.5 * (fRanges.at(PlaneNo).find(fExitWire.at(PlaneNo))->second.first +
                  fRanges.at(PlaneNo).find(fExitWire.at(PlaneNo))->second.second);
}

//----------------------------------------------------------------------------------------------------------------

std::vector<std::pair<geo::WireID, geo::WireID> >
lasercal::LaserROI::WireRanges(const TVector3 &StartPosition, const TVector3 &EndPosition) {
